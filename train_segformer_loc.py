...
import timeit
import cv2
from model.gfmodel import GFformer_one
...
class ValData(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        img = torch.from_numpy(img.transpose((2, 0, 1))).float().copy()
        msk = torch.from_numpy(msk.transpose((2, 0, 1))).long().copy()
...
def evaluate_val(data_val, best_score, model, snapshot_name, current_epoch):
    ...
    if d > best_score:
        torch.save({
            ...
            'best_score': d,
        }, path.join(models_folder, snapshot_name + '_best2'))
        best_score = d
...
def train_epoch(current_epoch, seg_loss, model, optimizer, scheduler, train_data_loader):
    ...
    for i, sample in enumerate(iterator):
        ...
        # out = F.interpolate(out, scale_factor=2)
        # out = F.interpolate(out, scale_factor=2)
        # print("output_shape____", out.shape, "input_shape", imgs.shape)
...
if __name__ == '__main__':
    ...
    snapshot_name = 'GFformer_loc_{}_0'.format(seed)
    ...
    train_data_loader = DataLoader(data_train, batch_size=batch_size, num_workers=5, shuffle=True, pin_memory=False,
                                   drop_last=True)
